...
if [ -z "$DD_TARGET_DIRECTORY" ] || [ -z "$REPOS" ] || [ -z "$BACKUP_LEVEL" ]; then
  echo "Error: DD_TARGET_DIRECTORY, REPOS, and BACKUP_LEVEL must be set."
  exit 1
fi
...
# Convert comma-separated REPOS string into an array
