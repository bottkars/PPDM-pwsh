...
func init() {
        ...
        flag.StringVar(&password, "password", "", "Password for authentication (required)")
        flag.Parse()
...
func runDDFSrc(app *tview.Application, env map[string]string) {
        ...
        // Build nsriscsi command arguments
        args := []string{
                "-h", "127.0.0.1",
                "-u", env["DD_USERNAME"],
                "-p", password,
                ...
                "-P", "1",
                "-d", env["DD_IP"],
                "-L", "/mnt/" + fqdn + env["DD_STORAGE_UNIT"],
                "-s", env["DD_STORAGE_UNIT"],
        }

        cmd := exec.Command("/opt/dpsapps/fsagent/bin/nsriscsi", args...)
        output, err := cmd.CombinedOutput()

        logFileName := fmt.Sprintf("nsriscsi-%s.log", time.Now().Format("2006-01-02"))
